...
<script>
    export default {
        ...
        computed: {
            localPageCount () {
                return this.pageCount || Math.ceil(this.totalCount / this.perPage)
            },
            selected: function() {
                return this.value;
            },
            pages: function () {
                let items = {}
                if (this.localPageCount <= this.pageRange) {
                    for (let index = 0; index < this.localPageCount; index++) {
                        let page = {
                            index: index,
                            content: index + 1,
                    ...
                    if (selectedRangeHigh >= this.localPageCount) {
                        selectedRangeHigh = this.localPageCount - 1;
                        selectedRangeLow = selectedRangeHigh - this.pageRange + 1;
                    }

                    for (let i = selectedRangeLow; i <= selectedRangeHigh && i <= this.localPageCount - 1; i++) {
                        setPageItem(i);
                    }
                    ...
                    if (selectedRangeHigh + 1 < this.localPageCount - this.marginPages) {
                        setBreakView(selectedRangeHigh + 1)
                    }

                    // 3rd - loop thru high end of margin pages
                    for (let i = this.localPageCount - 1; i >= this.localPageCount - this.marginPages; i--) {
                        setPageItem(i);
                    }
                }
                return items
        ...
        methods: {
            ...
            nextPage() {
                if (this.selected >= this.localPageCount) return
            ...
            lastPageSelected() {
                return (this.selected === this.localPageCount) || (this.localPageCount === 0)
            },
            selectFirstPage() {
                if (this.selected <= 1) return
            ...
            selectLastPage() {
                ...
                this.$emit('input', this.localPageCount)
                this.clickHandler(this.localPageCount)
            }
        }
    }
</script>
